# LAB211 Assignment - Handy Expense Program
...
- Prompt users input information of the expense (ID, Date, Quantity, Content)
- ID auto increase, ID = ID Max + 1 in the next time.
- Add expense to list.
...
- If ID does not exist, display on the screen: "Delete an expense fail"
...
### 3. Display all expenses (Option 2):
```
...
```
- **Input:** `list`: list of all expenses
- **Return values:** void.
...
- **Input:**
  ...
  - `exp`: The expense that user's want to delete.
